...
// Query data ruang + filter pencarian
$qRuang = "SELECT * FROM ruang";
if (!empty($keyword)) {
    $qRuang .= " WHERE nama_ruang LIKE '%$safeKeyword%' 
                 OR kode_ruang LIKE '%$safeKeyword%' 
                 OR keterangan LIKE '%$safeKeyword%'";
}

$resultRuang = mysqli_query($connect, $qRuang) or die(mysqli_error($connect));
?>
  ...
  <div class="page-inner py-5">
    ...
    <div class="text-center py-5">
      <h2 class="fw-bold mb-2 mt-4 text-dark display-5">
        <i class="bi bi-door-open-fill text-primary me-2"></i> Halaman Ruang
      </h2>
      <h5 class="text-muted">Daftar data ruang penyimpanan barang</h5>
    </div>
          ...
          <table id="dataTable" class="table table-hover align-middle text-center">
            ...
            <tbody>
              ...
              if (mysqli_num_rows($resultRuang) > 0):
                while ($item = $resultRuang->fetch_object()):
              ?>
                <tr>
                  <td><?= $no ?></td>
                  <td>
                    <span class="badge bg-info text-dark px-3 py-2">
                      <?= htmlspecialchars($item->kode_ruang) ?>
                    </span>
                  </td>
                  <td class="fw-semibold"><?= htmlspecialchars($item->nama_ruang) ?></td>
                  <td><?= htmlspecialchars($item->keterangan) ?></td>
                  <td>
                    <a href="./edit.php?id_ruang=<?= $item->id_ruang ?>" 
                       class="btn btn-sm btn-outline-warning me-1 shadow-sm">
                      <i class="bi bi-pencil-square"></i>
                    </a>
                    <a href="../../action/ruang/destroy.php?id_ruang=<?= $item->id_ruang ?>"
